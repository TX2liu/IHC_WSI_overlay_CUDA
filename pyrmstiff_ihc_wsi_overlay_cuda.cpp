...
int main(int argc,char *argv[])
{
    ...
    if((num_level !=20) && (num_level !=10) && (num_level !=40)){
        cerr << "倍率选择出错， 必须要是 20 或者 10 或者 40"  << endl;
        return -1;
    }
    double num_Rate = num_level==20 ? 1. : (num_level==40 ? 0.5 : 2.);
    ...
    tiffReader ref_reader(referenc_img_tile_folder);
    tiffReader flt_reader(template_img_tile_folder);

    int ref_whole_width = ref_reader.getWholeImageWidth(0);
    int ref_whole_height= ref_reader.getWholeImageHeight(0);
    int flt_whole_width = flt_reader.getWholeImageWidth(0);
    int flt_whole_height= flt_reader.getWholeImageHeight(0);
    ...
    string ss_ID = ss.str();
    std::string pyDown_path = pyrDown_base + "/" + ss_ID;
    cout << "pyDown_path is " << pyDown_path << endl;
    cout << "pyDownPath:" <<ss_ID << endl;
    ...
    const int cDepth = 4;
    std::vector<tiffWriter> mriw_vec(max_level_count);
    std::vector<uint32_t> level_whole_width(max_level_count,0);
    for(size_t idx=0; idx<max_level_count; idx++){
        std::string out_fn = pyDown_path + "/" + to_string(idx) + ".tiff";
        mriw_vec[idx].openFile(out_fn);
        mriw_vec[idx].setJPEGQuality(70);
        mriw_vec[idx].setTileSize(tilesize);
        mriw_vec[idx].setTileDepth(cDepth);
        
        uint32_t level_width = (ref_whole_width)>>idx;
        uint32_t level_height = ref_whole_height>>idx;
        level_whole_width[idx] = level_width;

        cout << "whole width and height at level : " << idx << " is " << level_width << ", " << level_height << endl;

        mriw_vec[idx].writeImageInformation(level_width, level_height);
    }
    ...
    for(int row_ind=0; row_ind<fix_row_num; row_ind++){
       ...
       // #pragma omp parallel for collapse(2)
        for(int col_ind=0; col_ind<tile_col_num_0; col_ind++){
            for(int k_ind=0; k_ind<4; k_ind++){
                ...
                cv::Mat flt_img = flt_reader.getLevel0TileFast(row_Idx, col_ind);
                cv::Mat ref_img = ref_reader.getLevel0TileFast(row_Idx, col_ind);

                int row_start = k_ind*tilesize;
                int col_start = col_ind*tilesize; 

                ref_img.copyTo(ref_big_image(cv::Rect(col_start, row_start, tilesize, tilesize)));
                flt_img.copyTo(flt_big_image(cv::Rect(col_start, row_start, tilesize, tilesize)));
      ...
      //  #pragma omp parallel for 
        for(int row=0; row<fix_row_len; row++){
            ...
            uchar* over_ptr = overlay_ihc.ptr<uchar>(row);
        ...
        vector<uint> pos_vec(numThreads);
        vector<vector<uchar>> img_encode_vec(numThreads);
        for(int tile_i=0; tile_i<tile_row_num_0; tile_i++){ // 1024是tile_size的4倍
            int tile_j = 0;
            for(tile_j=0; tile_j<tile_col_num_0; tile_j +=numThreads){ 

               // #pragma omp parallel for num_threads(numThreads)
                for(int jsub=0; jsub<numThreads; jsub++){
                    if(tile_j+jsub<tile_col_num_0){
                        int x = ((tile_j+jsub)<<tile_shift);
                        int x1 = ((tile_j+jsub+1)<<tile_shift);
                        int y = (tile_i<<tile_shift);
                        int y1 = ((tile_i+1)<<tile_shift);
                        cv::Mat tile_img;
                        cv::cvtColor(vcon_row(Range(y, y1), Range(x, x1)), tile_img, COLOR_BGRA2RGBA);
                        pos_vec[jsub] = (tile_i+row_ind*tile_row_num_0)*tile_col_num_0 + tile_j + jsub;
                        cv::imencode(".png", tile_img, img_encode_vec[jsub]);
                    }
                }

                for(int jsub=0; jsub<numThreads&&tile_j+jsub<tile_col_num_0; jsub++){
                    mriw_vec[0].writeBaseImagePartToTIFFTile_compress(img_encode_vec[jsub].data(), pos_vec[jsub], img_encode_vec[jsub].size());
                }
            }

            for(; tile_j<tile_col_num_0; tile_j++){ 
                vector<uchar> img_encode;
                uint pos = (tile_i+row_ind*tile_row_num_0)*tile_col_num_0 + tile_j;
                ...
                cv::imencode(".png", tile_img, img_encode);
                mriw_vec[0].writeBaseImagePartToTIFFTile_compress(img_encode.data(), pos, img_encode.size());
            }
        }
        ...
        // 其他级
        // for other level 1 to 10
        for(int level_i=1; level_i<max_level_count; level_i++){

            pyrsize_rows = fix_row_len>>level_i;  // 每次只右移一位
            pyrsize_cols = level_whole_width[level_i];
            tile_row_num_level = (pyrsize_rows + tilesize-1)>>tile_shift;
            tile_col_num_level = (pyrsize_cols + tilesize-1)>>tile_shift;
            
            if(level_i<=2){ // 不能小于256行， 前两级都是512,256，因此不需要行与行进行拼接之后在进行判断

                cv::resize(vcon_row, vcon_row, Size(pyrsize_cols, pyrsize_rows));  //直接resize成513或者257

                // level_tile_count[level_i]++;  // 在row_count=0是已经进行切图了，因此这里需要先进行加1

                for(int tile_i=0; tile_i<tile_row_num_level; tile_i++){ // 1024是tile_size的4倍
                    int tile_j = 0;
                    for(tile_j=0; tile_j<tile_col_num_level; tile_j +=numThreads){ 

                        // 以后新生成的所有图应该都是宽高都是1024的整数倍，这样就可以用这部分的程序；
                        // #pragma omp parallel for num_threads(numThreads)
                        // for(int jsub=0; jsub<numThreads; jsub++){
                        //     if(tile_j+jsub<tile_col_num_level){
                        //         int x = ((tile_j+jsub)<<tile_shift);
                        //         int x1 = ((tile_j+jsub+1)<<tile_shift);
                        //         int y = (tile_i<<tile_shift);
                        //         int y1 = ((tile_i+1)<<tile_shift);
                        ...
                        //         pos_vec[jsub] = (level_tile_count[level_i]*tile_row_num_level+tile_i)*tile_col_num_level + tile_j + jsub;
                        ...
                        for(int jsub=0; jsub<numThreads; jsub++){
                            if(tile_j+jsub<tile_col_num_level){
                                ...
                                int y1 = ((tile_i+1)<<tile_shift);
                                int x = ((tile_j+jsub)<<tile_shift);
                                bool is_end_tile = x+tilesize<=pyrsize_cols;
                                int x1;
                                if(is_end_tile){
                                    x1 = x+ tilesize;
                                    cv::Mat tile_img;
                                    ...
                                    cv::Mat tile_img_allOne(tilesize, tilesize, CV_8UC4, cv::Scalar::all(255));
                                    tile_img.copyTo(tile_img_allOne(Range(0,tilesize), Range(0,x1-x)) );
                                    pos_vec[jsub] = (level_tile_count[level_i]*tile_row_num_level+tile_i)*tile_col_num_level + tile_j + jsub;
                                    cv::imencode(".png", tile_img_allOne, img_encode_vec[jsub]);
                                }
                        ...
                        for(int jsub=0; jsub<numThreads&&tile_j+jsub<tile_col_num_level; jsub++){
                            mriw_vec[level_i].writeBaseImagePartToTIFFTile_compress(img_encode_vec[jsub].data(), pos_vec[jsub], img_encode_vec[jsub].size());
                        }
                    }

                    for(; tile_j<tile_col_num_level; tile_j++){ 
                        vector<uchar> img_encode;
                        uint pos = (level_tile_count[level_i]*tile_row_num_level+tile_i)*tile_col_num_level + tile_j;
                        ...
                        cv::imencode(".png", tile_img, img_encode);
                        mriw_vec[level_i].writeBaseImagePartToTIFFTile_compress(img_encode.data(), pos, img_encode.size());
                    }
                }
                ...
                if(tmp_vcon_row_vec[level_i].rows>0){
                    ...
                    if(tmp_vcon_row_vec[level_i].rows > tilesize){//判断是否超过tile_size，超过tile_size,才能进行切图
                        ...
                        int tile_j = 0;
                        if(tile_col_num_level>numThreads){
                            for(tile_j=0; tile_j<tile_col_num_level; tile_j +=numThreads){ 
                                ...
                                for(int jsub=0; jsub<numThreads; jsub++){
                                    if(tile_j+jsub<tile_col_num_level){
                                        ...
                                        if(is_end_tile){
                                            x1 = x+ tilesize;
                                            cv::Mat tile_img;
                                            cv::cvtColor(tmp_vcon_row_vec[level_i](Range(0, tilesize), Range(x, x1)), tile_img, COLOR_BGRA2RGBA);
                                            pos_vec[jsub] = (level_tile_count[level_i])*tile_col_num_level + tile_j + jsub;
                                            cv::imencode(".png", tile_img, img_encode_vec[jsub]);
                                            ...
                                            cv::Mat tile_img;
                                            cv::cvtColor(tmp_vcon_row_vec[level_i](Range(0, tilesize), Range(x, pyrsize_cols)), tile_img, COLOR_BGRA2RGBA);
                        ...
                        for(; tile_j<tile_col_num_level; tile_j++){ 
                            vector<uchar> img_encode;
                            uint pos = (level_tile_count[level_i])*tile_col_num_level + tile_j;
                            ...
                            if(is_end_tile){
                                ...
                                cv::Mat tile_img_allOne(tilesize, tilesize, CV_8UC4, cv::Scalar::all(255));
                                tile_img.copyTo(tile_img_allOne(Range(0,tilesize), Range(0,x1-x)) );
                                cv::imencode(".png", tile_img_allOne, img_encode);
                            }
    ...
    for(int level_i=3; level_i<max_level_count; level_i++){

        if(tmp_vcon_row_vec[level_i].cols>tilesize){//判断列是否超过256, 因为行必定小于256了(如果超过256,在前边就已经截取了)
            int tile_col_num_level = (level_whole_width[level_i] + tilesize-1)>>tile_shift;
                ...
                for(tile_j=0; tile_j<tile_col_num_level; tile_j +=numThreads){ 
                    ...
                    for(int jsub=0; jsub<numThreads; jsub++){
                        if(tile_j+jsub<tile_col_num_level){
                            ...
                            cv::Mat tile_img;
                            cv::cvtColor(tmp_vcon_row_vec[level_i].colRange(x, xend), tile_img, COLOR_BGRA2RGBA);
                            cv::Mat tile_img_allOne(tilesize, tilesize, CV_8UC4, cv::Scalar::all(255));
                            tile_img.copyTo(tile_img_allOne(Range(0,tile_img.rows),Range(0,tile_img.cols)) );
                            pos_vec[jsub] = (level_tile_count[level_i])*tile_col_num_level + tile_j + jsub;
                            cv::imencode(".png", tile_img_allOne, img_encode_vec[jsub]);
                        }
                    }
            ...
            if(tmp_vcon_row_vec[level_i].rows>0&&tmp_vcon_row_vec[level_i].cols>0){

                vector<uchar> img_encode;
                uint pos = level_tile_count[level_i];

                cv::Mat tile_img;
                cv::cvtColor(tmp_vcon_row_vec[level_i], tile_img, COLOR_BGRA2RGBA);
                cv::Mat tile_img_allOne(tilesize, tilesize, CV_8UC4, cv::Scalar::all(255));
                tile_img.copyTo(tile_img_allOne(Rect(0,0,tile_img.cols, tile_img.rows)) );
                cv::imencode(".png", tile_img_allOne, img_encode);
                mriw_vec[level_i].writeBaseImagePartToTIFFTile_compress(img_encode.data(), pos, img_encode.size());
